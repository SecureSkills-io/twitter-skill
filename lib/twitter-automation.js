...
async function postTweet() {
  ...
  // Navigate to home first to ensure we're logged in
  await page.goto('https://x.com/home', { waitUntil: 'networkidle' });
  await page.waitForTimeout(2000);
  
  // Click compose button
  const composeBtn = await page.$('a[href="/compose/post"]') 
    || await page.$('[data-testid="SideNav_NewTweet_Button"]')
  ...
  const textbox = await page.$('[data-testid="tweetTextarea_0"]') 
    ...
    || await page.$('[data-testid="tweetButton"]').then(btn => btn ? page.$('[role="textbox"]') : null);
